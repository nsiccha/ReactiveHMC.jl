...
export const Theme: ThemeConfig = {
  ...
  enhanceApp({ app, router, siteData }) {
    ...
    // re-process + .htmxo-embed link rewriting. ReactiveHMC defaults
    // the proxy prefix to `/live-reactivehmc` (matches the Vite proxy in
    // config.mts and the committed recordings under
    // public/live-reactivehmc/).
    setupHtmxoEmbed(router, { proxyPrefix: '/live-reactivehmc' });
  }
}
